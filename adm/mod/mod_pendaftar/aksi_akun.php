...
if ($act=='upass'){
  	$qp = mysqli_query("UPDATE pendaftar SET psPass='$_POST[pass]' WHERE psNISN='$_POST[nisn]'");
  ...
  mysqli_query("UPDATE pendaftar SET psSt_Akses='0' WHERE psNISN='$_GET[id]'");
  echo "<script>window.location=('../../media.php?page=akun')</script>";

}elseif ($act=='unblok'){

  mysqli_query("UPDATE pendaftar SET psSt_Akses='1' WHERE psNISN='$_GET[id]'");
  echo "<script>window.location=('../../media.php?page=akun')</script>";

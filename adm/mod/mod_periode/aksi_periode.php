...
if ($act=='tambah'){
  	$q= mysqli_query("INSERT INTO periode (taPeriode,taTahun,taAktif)VALUES('$_POST[periode]','$_POST[tahun]','0')");
    if ($q){
   ...
   $q= mysqli_query("UPDATE periode SET taPeriode='$_POST[periode]',
                                       taTahun='$_POST[tahun]'
                                  WHERE taId='$_POST[id]'");
    ...
    if ($pAktif[taId]==$_GET[id]){
      ...
      $q = mysqli_query("DELETE FROM periode WHERE taId='$_GET[id]' AND taAktif='0'");
      echo "<script>window.location=('../../media.php?page=periode')</script>";   
    }
}elseif ($act=='aktif'){
    $q1 = mysqli_query("UPDATE periode SET taAktif='0'");
    $q2 = mysqli_query("UPDATE periode SET taAktif='1' WHERE taId='$_GET[id]'");
    echo "<script>window.location=('../../media.php?page=periode')</script>";   
}
?>

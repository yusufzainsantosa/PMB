...
function getNoPeserta(){
	...
	$q = mysqli_fetch_array(mysqli_query("SELECT MAX(psNoPeserta) as x FROM pendaftar WHERE psTglDaftar='$now'"));
	$d = $q[x];
	$not = substr($d, 8,3);
...
function getStatusBio($id){
	$qq = mysqli_fetch_array(mysqli_query("SELECT a.psSt_Bio FROM pendaftar a
	   											WHERE a.psNISN='$id'
	   											"));
	$st = $qq[psSt_Bio];
   return $st;
...
function getStatusNilai($id){
	$qx = mysqli_fetch_array(mysqli_query("SELECT a.psNISN, 
						   								 c.bingStatus,
						   								 d.indoStatus,
						   								 e.ipaStatus,
						   								 f.ipsStatus,
...
function getPeriodeAktif(){
	...
	$pAktif = mysqli_fetch_array(mysqli_query("SELECT taId,taTahun FROM periode WHERE taAktif='1'"));
	return $pAktif;
...
function getJumlah($tabel,$term){
	...
	$jRec = mysqli_num_rows(mysqli_query("SELECT * FROM $tabel $term"));
	return $jRec;
